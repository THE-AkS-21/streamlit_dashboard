...
def get_dashboard_metadata():
    """Fetch filter metadata for dashboard dropdowns with caching"""
    return db.execute_query(DashboardQueries.GET_DASHBOARD_FILTER_METADATA)

def show_dashboard():
    """Render the main dashboard UI page"""
    render_content_start()

    st.markdown("## Bombay Shaving Company Dashboard")

    # Load dashboard metadata (categories, subcategories, skus, last_date)
    ...
    # Process and display dashboard data if 'Generate Report' is clicked
    if plot_button:
        try:
